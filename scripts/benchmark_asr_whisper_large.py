"""
ASR Benchmark — Faster-Whisper-Large-V3-Turbo on LibriSpeech test-clean
Metrics: WER (Word Error Rate), RTFx (Inverse Real-Time Factor)

This is the model used in the integrated voice assistant app (asr_server.py).
Model is downloaded automatically from HuggingFace on first run (~1.5 GB).

Required env packages:
  faster-whisper, datasets, jiwer, numpy, pandas, torch

Usage:
  python benchmark_asr_whisper_large.py
"""
...
from config import ASR_LARGE_TURBO_ID, ASR_BENCH_PATH, OUT_TABLES

# ── settings
DEVICE      = "cuda" if torch.cuda.is_available() else "cpu"
COMPUTE_FP  = "int8_float16" if DEVICE == "cuda" else "int8"
...
def main():
    ...
    print(f"\nLoading Faster-Whisper-Large-V3-Turbo (device={DEVICE}) ...")
    print("First run will auto-download ~1.5 GB from HuggingFace ...")
    fw_model = WhisperModel(ASR_LARGE_TURBO_ID, device=DEVICE, compute_type=COMPUTE_FP)
    ...
    # ── Save results
    result = {"Faster-Whisper-Large-V3-Turbo": {"WER%": round(fw_wer * 100, 2), "RTFx": round(fw_rtfx, 2)}}
    df = pd.DataFrame(result).T
    out_path = OUT_TABLES / "asr_whisper_large_benchmark.csv"
    df.to_csv(out_path)
    print(f"\nSaved to {out_path}")
