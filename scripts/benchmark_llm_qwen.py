"""
LLM Benchmark — Qwen2.5 (1.5B vs 3B, Q4_K_M) on MMLU 10-subject subset
Hyperparameter sweep: n_ctx in [1024, 2048, 5012]
...
from config import LLM1_PATH, LLM1_5B_PATH, MMLU_BENCH_DIR, MMLU_SUBJECTS, OUT_TABLES
...
def run_ctx(model_name: str, model_path: str, ctx: int, mmlu_data: dict) -> dict:
    """Load model with given n_ctx, run full MMLU sweep, return result dict."""
    from llama_cpp import Llama

    print(f"\n{'='*60}")
    print(f"Model: {model_name}  |  n_ctx = {ctx}")
    print(f"{'='*60}")

    if _CUDA:
        torch.cuda.reset_peak_memory_stats()

    print(f"Loading {model_name} (n_ctx={ctx}, n_gpu_layers={N_GPU_LAYERS}) ...")
    llm = Llama(
...
def main():
    ...
    # ── Sweep: both models × all ctx sizes
    rows = []
    for model_name, model_path in QWEN_MODELS:
        for ctx in CTX_SIZES:
            rows.append(run_ctx(model_name, model_path, ctx, mmlu_data))

    # ── Save results
    df = pd.DataFrame(rows).set_index(["model", "n_ctx"])
    out_path = OUT_TABLES / "llm_qwen_benchmark.csv"
    df.to_csv(out_path)
